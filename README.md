...
本仓库中的部分思路、流程设计与实现方式参考/借鉴了开源社区项目与作者实践，并在真实业务场景中做了重构与工程化落地。  
特别感谢（包括但不限于）：

- @baoyu（方法论与工作流启发）
- OpenClaw / Claude Code / Codex 相关生态中的公开实践与示例

如你是被引用或借鉴的作者，但此处未完整署名，欢迎提 Issue 或 PR 补充链接，我会及时更新致谢名单。  
本项目仅代表个人整理与实现，不代表上述作者或项目对本仓库内容的背书。
