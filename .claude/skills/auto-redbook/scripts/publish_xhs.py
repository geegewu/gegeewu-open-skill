...
class LocalPublisher:
    ...
    def init_client(self):
        ...
        def sign_func(uri, data=None, a1="", web_session=""):
            # 使用 cookie 中的 a1 值
            cookies_a1 = cookies.get('a1', '')
            return local_sign(uri, data, a1=cookies_a1 or a1)
    ...
    def resolve_topics(self, tag_keywords: list) -> list:
        """将 tag 关键词列表解析为结构化 topic 对象
        
        通过 XHS 搜索 API 查找每个关键词对应的官方话题，
        返回可直接传给 create_image_note(topics=...) 的结构化列表。
        等同于用户在编辑器中输入 # 后从下拉列表选择话题的过程。
        """
        resolved = []
        for keyword in tag_keywords:
            ...
            if not keyword:
                continue
            try:
                results = self.client.get_suggest_topic(keyword)
                if results:
                    # 优先精确匹配，否则取第一个（按热度排序）
                    best = None
                    for r in results:
                    ...
                    if not best:
                        best = results[0]
                    # XHS API 要求 type='topic' 才能渲染为可点击标签
                    # get_suggest_topic 返回 type='official'，需要强制修正
                    # Ref: https://github.com/ReaJason/xhs/issues/106
                    best['type'] = 'topic'
                    resolved.append(best)
                    view_num = best.get('view_num', 0)
                    if view_num >= 100_000_000:
                        view_str = f"{view_num / 100_000_000:.1f}亿"
                    elif view_num >= 10_000:
                        view_str = f"{view_num / 10_000:.1f}万"
                    else:
                        view_str = str(view_num)
                    print(f"  ✓ Topic '{keyword}' → #{best['name']}（{view_str}浏览）")
                else:
                    print(f"  ⚠️ Topic '{keyword}' 未找到匹配话题，跳过")
            except Exception as e:
                print(f"  ⚠️ Topic '{keyword}' 搜索失败: {e}")
        return resolved
    
    def _confirm_topics(self, note_id: str, title: str, desc: str, topics: list):
        """发布后立即触发一次 update，让 topics 从草稿状态变为可点击标签"""
        try:
            print("  🔄 正在确认话题标签（update note）...")
            
            # 获取帖子详情拿 image 信息
            note_detail = self.client.get_note_by_id(note_id)
            note = note_detail.get('note', note_detail)
            images_raw = note.get('image_list', [])
            ...
            business_binds = {
                "version": 1,
                "noteId": int(note_id, 16) if len(note_id) == 24 else 0,
                "noteOrderBind": {},
            ...
            result = self.client.post("/web_api/sns/v2/note", data, headers=headers)
            print("  ✅ 话题标签确认成功")
            return result
        except Exception as e:
            print(f"  ⚠️ 话题确认失败（不影响发布）: {e}")
            return None
    
    def publish(self, title: str, desc: str, images: List[str], 
                is_private: bool = False, post_time: str = None,
                topics: list = None) -> Dict[str, Any]:
        """发布图文笔记"""
        print(f"\n🚀 准备发布笔记（本地模式）...")
        print(f"  📌 标题: {title}")
        ...
        try:
            ...
            # 如果发布成功且有 topics，触发一次 update 确认话题标签
            if note_id and topics:
                time.sleep(2)  # 等 XHS 后端处理完
                self._confirm_topics(note_id, title, desc, topics)
            ...
            elif any(k in error_msg.lower() for k in ['timeout', 'timed out', 'connection', 'network', 'proxy']):
                print("\n💡 网络/连接错误排查建议：")
                print("1. 检查当前网络、代理和 DNS")
                print("2. 若你所在网络有地域/IP限制，尝试切换网络出口")
                print("3. 若使用 --api-mode，先验证 API 服务可达")
            elif any(k in error_msg.lower() for k in ['403', 'forbidden', 'risk', '风控', 'captcha']):
                print("\n💡 平台风控/权限错误排查建议：")
                print("1. 当前账号可能触发风控，稍后重试")
                print("2. 在浏览器端确认账号状态正常")
                print("3. 必要时重新登录并更新 Cookie")
